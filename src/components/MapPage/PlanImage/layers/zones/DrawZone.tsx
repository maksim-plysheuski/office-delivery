...
import { setDrawZoneFloor } from '@/features/modal/floorPlan/model/slice'
...
export const DrawZone: React.FC<DrawZoneProps> = ({
  points,
  flattenedPoints,
  isFinished,
  currentFloorData,
  ...
  const handleContextMenu = (e: Konva.KonvaEventObject<MouseEvent>) => {
    ...
    if (showSaveButton) {
      ...
      dispatch(setDrawZoneFloor(true))
    }
    if (!showSaveButton) {
  ...
  const onSubmitZone = async () => {
    ...
    try {
      ...
      const zoneHeight = Number(localStorage.getItem('zoneHeight')) ?? 0

      const response = await createUpdZone({
        id: null,
        floorId: currentFloorData?.id ?? 0,
        initialHeight: currentFloorData?.initialHeight ?? 0,
        finalHeight: zoneHeight,
        polygon: {
          points: zoneToSend,
        },
        zoneName: zoneName,
      ...
      const updatedFloorData = {
        ...elementInfo.floor,
        zones: [...prevFloorData.zones, response],
      }
      ...
      const successMessage = isEditMode
        ? t('modalTitle.zoneEditSuccess')
        : t('modalTitle.zoneAddSuccess')
      ...
      const errorMessage = isEditMode
        ? t('modalTitle.zoneEditError')
        : t('modalTitle.zoneAddError')
      ...
      localStorage.removeItem('zoneHeight')
